# Let L be a list with k elements, where each element is a sorted list of integers. Write a Python function mergeKLists(L) that merges all the lists into a single sorted list and returns it. Try to write a solution that runs in O(nlogk) time complexity, where n is the total number of integers in all k lists combined.
# nlogk O(nlogk) time complexity, where n is the total number of integers in all k lists combined.
